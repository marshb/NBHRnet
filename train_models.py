...
from utils.batchDataGenerator_fusemodel import BatchPPGGenerator  # PPG signal is R60
...
def scheduler(epoch):
    ...
    if epoch % 30 == 0 and epoch != 0:
        lr = K.get_value(model_PPG.optimizer.lr)
        K.set_value(model_PPG.optimizer.lr, lr * 0.3)
        print("lr changed to {}".format(lr * 0.3))
    return K.get_value(model_PPG.optimizer.lr)
...
if __name__ == "__main__":
    ...
    model_PPG = model_class.PPG_extractor_model()
    ...
    model_PPG.compile(loss=pearson_mse, optimizer=RMSprop(lr=lr), metrics=["mse"])
    ...
    history = model_PPG.fit_generator(generator=train_generator,
                                      epochs=NUM_EPOCH,
    ...
    model_PPG.save(path_model_saved)
